...
void WebSocketServer::set_on_listening(std::move_only_function<void()> callback) {
  if (impl_) {
    impl_->set_on_listening(std::move(callback));
  }
}

void WebSocketServer::set_on_error(std::move_only_function<void(Status)> callback) {
  if (impl_) {
